...
<template>
  <div class="bg-white rounded-lg shadow-sm p-4">
    ...
    <div v-else-if="popularPosts.length === 0" class="text-gray-500 text-sm text-center py-4">
      暂无热门文章
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        <RouterLink
          :to="`/posts/${post.slug}`"
          class="block hover:bg-gray-50 -m-2 p-2 rounded transition-colors"
        >
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1 min-w-0">
              <h4 class="text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2 leading-5">
                {{ post.title }}
              </h4>
